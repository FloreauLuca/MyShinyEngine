...
namespace shiny
{
  void GraphicsPipeline::CreateGraphicsPipeline(VkDevice* logical_device, VkDescriptorSetLayout* descriptor_set_layout, VkExtent2D* extent, VkFormat* format) {
    ...
    auto vertShaderCode = ReadFile("./Data/shader_vert.spv");
    auto fragShaderCode = ReadFile("./Data/shader_frag.spv");

    vert_shader_module_ = CreateShaderModule(vertShaderCode);
    frag_shader_module_ = CreateShaderModule(fragShaderCode);
    ...
    //vertShaderStageInfo.pSpecializationInfo = "main"; Use for constant
